...
export default class AsanaProvider {
    ...
    getTasksByWorkspace(workspaceID,assignID){
        ...
        return new Promise(function(resolve, reject) {
            fetch('https://app.asana.com/api/1.0/tasks?workspace='+workspaceID+'&assignee='+assignID, {
                headers: that.header
            })
            .then(function(response) {
              return response.json();
    ...
    getAllProjectTasks(projectID){
        ...
        return new Promise(function(resolve, reject) {
            fetch('https://app.asana.com/api/1.0/projects/'+projectID+'/tasks?opt_fields=name,completed_at', {
                headers: that.header
            })
            .then(function(response) {
              return response.json();
